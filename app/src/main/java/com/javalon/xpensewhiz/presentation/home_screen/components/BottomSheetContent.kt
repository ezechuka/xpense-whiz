...
fun AddEntryComponent(bottomSheetScaffoldState: BottomSheetScaffoldState, homeViewModel: HomeViewModel = hiltViewModel()) {
    ...
    val expenseAmount = remember { homeViewModel.expenseAmount }
    ...
    Column(
        ...
        KeypadComponent(homeViewModel, bottomSheetScaffoldState) {
            homeViewModel.setExpense(it)
        }
    }

    CategoryDialog(showDialog = homeViewModel.dialogState.value, onDismiss =  {
        homeViewModel.setDialogState(false)
    })
}
...
fun PreviewBottomSheetContent() {
    AddEntryComponent(bottomSheetScaffoldState = rememberBottomSheetScaffoldState())
}